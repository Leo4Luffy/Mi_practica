...
<section style="text-align: justify;"><p class="text-primary">Luego los datos sin procesar se transforman en características. Así por ejemplo usando el método de codificación activa<!-- (Una codificación activa es un proceso que transforma características categóricas o discretas en una matriz numérica.)-->, el mes de parto se conviertió en una característica denominada época de parto (sequía, transición y lluvia). La misma se convierte luego en valores numericos (1, 2 y 3, respectivamente) para que la regresión de machine learning puede comprenderlo mejor.</p></section>
...
<section style="text-align: justify;"><p class="text-primary">Estos son problemáticos porque pueden cambiar drásticamente las estimaciones y predicciones de ajuste, y porque su presencia puede ser una señal de que el modelo no logra capturar características importantes de los datos.</p></section>

<section style="text-align: justify;"><p class="text-primary">El boxplot es un método <!--típicamente representado por cuartiles e intercuartiles--> que ayuda a definir el límite superior e inferior más allá de los cuales cualquier dato que se encuentre será considerado como atípico. <!--El propósito mismo de este diagrama es identificar valores atípicos y descartarlos de la serie de datos antes de hacer cualquier observación adicional para que la conclusión del estudio proporcione resultados más precisos no influenciados por ningún extremo o valores anormales.--></p></section>
...
<section style="text-align: justify;"><p class="text-primary">La forma más fácil de entender una distribución es visualizarla. Un histrograma como el de la imagen es un ejemplo de dicha visualización. De igual forma, las propiedades de una distribución se pueden describir mediante estadística descriptiva como la media, la mediana, el sesgo y la curtosis.</p></section>

```{r Imagen octocat 2, fig.align = 'center', out.width = "150", eval = TRUE, echo = FALSE, message = FALSE}
...
```{r Estadisticos para averiguar la distribución del intervalo entre partos, echo = FALSE, eval = FALSE, message = FALSE}

Est_dist <- Base_datos_3 %>%
  summarise(Promedio = mean(IEP), Mediana = median(IEP), Sesgo = skewness(IEP), Curtosis = kurtosis(IEP))
```
...
<section style="text-align: justify;"><p class="text-primary">Por lo tanto, al observar el histograma con una asimetría positiva y una curtosis por encima de 3, se concluye que la distribución de los datos de intervalo entre partos en bovinos Romosinuano corresponde a una distribución sesgada a la derecha.</p></section>
