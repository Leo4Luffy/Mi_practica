...
<!--
Aquí (https://nicd.ncl.ac.uk/blog/posts/building-a-classification-model-with-tidymodels/) indican que el propósito de dividir los datos en conjuntos de entrenamiento y prueba es evitar un ajuste excesivo y permitirnos comprender cómo se desempeñará nuestro modelo elegido en datos nuevos e invisibles. Por esa razón, el conjunto de prueba no se usa para seleccionar el modelo o el ajuste del hiperparámetro del modelo.
-->
...
```{r, fig.align = 'center', out.width = '670', eval = FALSE, echo = FALSE, message = FALSE}

knitr::include_graphics('Imagenes/paquete_recipes.png')
```

<!--
Si bien los datos de IEP usados en este estudio no tienen valores ausentes (NA), aquí (https://www.cienciadedatos.net/documentos/59_machine_learning_con_r_y_tidymodels) mencionan lo importante y básico que es conocer el número de observaciones disponibles y si todas ellas estan completas, puesto que los valores NA pueden influir sobre los modelos de machine learning. También si es el caso de presenterse valores NA, recomienda llevar a cabo imputación (tratar de estimar los valores ausentes empleando el resto de información disponible) como parte del preprocesado de datos únicamente en los datos de entrenamiento.
-->

```{r Función que calcula el número de datos ausentes por variable, echo = FALSE, eval = FALSE, message = FALSE}

Base_datos_3 %>% map_dbl(.f = function(x){sum(is.na(x))}) # La función map_dbl es del paquete purrr.
...
plot_missing(
  ...
  title   = '',
  ggtheme = theme_bw(),
  theme_config = list(legend.position = '')
)
```
...
Reg_bas_mod <- linear_reg() %>% # Se especifica aqui la forma funcional (o el tipo) del modelo a ajustar usando la función linear_reg() del paquete parsnip. En este caso corresponde a modelo de regresión lineal.
...
Reg_bas_rec <- recipe(
  ...
  ) %>% # Aquí, la variable al lado izquierdo de "~" se considera la variabe respuesta, y las del lado derecho las variables predictoras. Se indico con "." todas las demás variables además de "IEP" como variables predictoras, y en "data" el conjunto de entrenamiento.
...
# NOTA: Aquí (https://www.cienciadedatos.net/documentos/59_machine_learning_con_r_y_tidymodels) indica que todo preprocesado de datos debe realizarse con los datos de entrenamiento y luego aplicarse a los otros tipos de datos, lo cual es muy importante para no violar la condición de que ninguna información procedente de los datos de prueba participen o influyan en el ajuste del modelo.
...
En esta pagina comparten el paso a paso del uso de tidymodels, mediante un modelo de regresión lineal y bosque aleatorio:
- https://hohenfeld.is/posts/an-introduction-to-tidymodels/.
-->
